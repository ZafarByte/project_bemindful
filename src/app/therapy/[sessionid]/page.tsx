...
export default function TherapyPage() {
  ...
  const processMessageToApi = async (text: string) => {
    if (!sessionId) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await processMessageToApi(currentMessage);
  };
  ...
  return (
    <div className="relative max-w-7xl mx-auto px-4">
      ...
      <Dialog open={!!stressPrompt && !showActivity} onOpenChange={(open) => !open && setStressPrompt(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>I noticed you're feeling {stressPrompt?.trigger}</DialogTitle>
            <DialogDescription>
              Would you like to try a {stressPrompt?.activity.title} to help you feel better?
              <br />
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => {
              if (stressPrompt?.originalMessage) {
                processMessageToApi(stressPrompt.originalMessage);
              }
      ...
      <Dialog open={showActivity} onOpenChange={(open) => {
        if (!open) {
        ...
        <DialogContent className="max-w-4xl h-[80vh]">
          <DialogHeader>
            <DialogTitle>{stressPrompt?.activity.title}</DialogTitle>
          </DialogHeader>
